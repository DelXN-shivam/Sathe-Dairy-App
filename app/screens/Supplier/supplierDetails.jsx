...
const SupplierDetail = () => {
  ...
  const fetchInwardTransactions = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch inward transactions: ${response.status}`);
      }
      const data = await response.json();
      console.log("All inward transactions:", data);
      
      // Filter transactions by supplier ID
      const supplierTransactions = data.filter(transaction => 
        transaction.supplierDetails?.supplierId === supplierId
      );
      console.log("Filtered transactions for supplier:", supplierTransactions);
      
      setInwardTransactions(supplierTransactions);
    } catch (err) {
  ...
  const handleTransactionPress = (transaction) => {
    ...
    console.log('Navigating to InwardDetail with ID:', transaction._id);
    navigation.navigate('InwardDetail', { 
      inwardId: transaction._id
    });
  };

  const handleEditSupplier = () => {
    navigation.navigate('UpdateSupplier', {
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center">
        ...
        <Text className="text-gray-600 mt-2">Loading supplier details...</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
              ...
              <View className="flex-row items-center flex-1">
                <View className="w-16 h-16 rounded-full bg-blue-500 flex justify-center items-center">
                  <Text className="text-2xl font-bold text-white">
                    {getInitials(supplierDetail.supplier.supplierName)}
                  </Text>
                </View>
                <View className="ml-4 flex-1">
                  <Text className="text-2xl font-bold text-gray-800">
                    {supplierDetail.supplier.supplierName}
                  </Text>
                  <View className="flex-row items-center mt-1">
                    <Icon name="card-account-details" size={18} color="#64748b" />
                    <Text className="text-gray-500 ml-2">
                      {supplierDetail.supplier.supplierGSTNo || "N/A"}
                    </Text>
                  </View>
                </View>
              </View>
            ...
            <View className="mt-4 space-y-2">
              <View className="flex-row items-center">
                ...
                <Text className="text-gray-600 ml-2">
                  {supplierDetail.supplier.supplierMobileNo || "N/A"}
                </Text>
              </View>
              <View className="flex-row items-center">
                <Icon name="map-marker-outline" size={18} color="#64748b" />
                <Text className="text-gray-600 ml-2">
                  {supplierDetail.supplier.supplierAddress || "N/A"}
                </Text>
              </View>
              <View className="flex-row items-center">
                <Icon name="email-outline" size={18} color="#64748b" />
                <Text className="text-gray-600 ml-2">
                  {supplierDetail.supplier.supplierEmailId || "N/A"}
                </Text>
              </View>
              <View className="flex-row items-center">
                <Icon name="card-account-details-outline" size={18} color="#64748b" />
                <Text className="text-gray-600 ml-2">
                  {supplierDetail.supplier.supplierPanNumber || "N/A"}
                </Text>
              </View>
              <View className="flex-row items-center">
                <Icon name="bank-outline" size={18} color="#64748b" />
                <Text className="text-gray-600 ml-2">
                  {supplierDetail.supplier.supplierAccountNumber || "N/A"} ({supplierDetail.supplier.supplierIFSCode || "N/A"})
                </Text>
              </View>
            </View>
          </>
      ...
      <View className="mb-8">
        <Text className="text-xl font-bold text-gray-800 mb-4">Inward Transactions</Text>
        
        {inwardTransactions.length > 0 ? (
          inwardTransactions.map((transaction, index) => (
            <TouchableOpacity 
              key={index} 
          ...
          <View className="bg-white rounded-xl p-6 items-center justify-center">
            ...
            <Text className="text-gray-500 mt-2">No inward transactions found</Text>
          </View>
        )}
      </View>
    </ScrollView>
